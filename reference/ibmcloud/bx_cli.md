...
If you're using quotations or exclamation marks as part of your query, make sure to escape each character by using `\`. For more information, see [Using quotation marks with strings in {{site.data.keyword.cloud_notm}} CLI](/docs/cli?topic=cli-quote-strings).
{: tip}
...
--sso
:   Specify this option to [log in with a federated ID](/docs/iam?topic=iam-federated_id&interface=ui). Using this option prompts you to authenticate with your single sign-on provider and enter a one-time passcode to log in.
